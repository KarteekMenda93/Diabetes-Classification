...
def main():
    ...
    def plot_metrics(metrics_list):
        if "Confusion_matrix" in metrics_list:
            ...
            confusion_matrix(model, xtest, ytest, display_labels=class_name)
            st.pyplot()
        if "ROC" in metrics_list:
            st.subheader("ROC")
            roc_curve(model, xtest, ytest)
            st.pyplot()
    ...
    if classifier == "Decision Tree":
        ...
        metrics = st.sidebar.multiselect("What metrics to plot?", ("Confusion_matrix", "ROC", "Precision Recall Curve"))
        if st.sidebar.button("Classify"):
            model = DecisionTreeClassifier(max_leaf_nodes=max_leaf_nodes, criterion=criterion,
    ...
    if classifier == "Neural Network":
        ...
        if st.sidebar.button("Classify"):
            model = MLPClassifier(activation=activation, solver=solver, alpha=alpha, learning_rate=learning_rate,
                                  max_iter=max_iter, hidden_layer_sizes=(8, 8, 8))
            model.fit(xtrain, ytrain)
            ypred = model.predict(xtest)
            st.write("Model Accuracy: ", model.score(xtest, ytest))
            st.write("Model Precision: ", precision_score(ytest, ypred, labels=class_name))
